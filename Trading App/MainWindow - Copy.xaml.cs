    ...
    public partial class MainWindowTest : Window
    {
        ...
        private void LogAdded(string logMessage)
        {
            if (!Thread.CurrentThread.IsBackground)
                ...
                Dispatcher.BeginInvoke(new LogHandler(AddLogs), new object[] { logMessage });
        }
        ...
        private async void btnEntry_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                apiProcessor.IsCEOrder = (bool)chkCallChecked.IsChecked;
                apiProcessor.IsPEOrder = (bool)chkPutChecked.IsChecked;
                apiProcessor.UserTransType = comboBoxBUYSELL.Text;
                apiProcessor.IsStrangle = Convert.ToBoolean(chkStrangleChecked.IsChecked);
                apiProcessor.Strike = Convert.ToInt32(txtStrike.Text);

                if (apiProcessor.IsStrangle)
                {
                    if (!string.IsNullOrEmpty(txtOTMDiff.Text))
                        apiProcessor.OtmDifference = Convert.ToInt32(txtOTMDiff.Text);
                    else
                        apiProcessor.OtmDifference = 0;
                }
